...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ============================
    // MOBILE NAV TOGGLE (public header — new nav-white)
    // ============================
    const mobileToggle = document.getElementById('mobileNavToggle');
    const publicNav = document.getElementById('navLinks');
    if (mobileToggle && publicNav) {
        mobileToggle.addEventListener('click', function () {
            const isOpen = publicNav.classList.toggle('active');
            mobileToggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
            document.body.style.overflow = isOpen ? 'hidden' : '';
        });
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape' && publicNav.classList.contains('active')) {
    ...
    // ============================
    // STICKY NAV-WHITE SHADOW
    // ============================
    const navWhite = document.querySelector('.nav-white');
    if (navWhite) {
        window.addEventListener('scroll', function () {
            navWhite.classList.toggle('nav-scrolled', window.pageYOffset > 50);
        }, { passive: true });
    }
