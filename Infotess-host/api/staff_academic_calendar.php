...
try {
    ...
    try {
        ...
        error_log("Academic Calendar: events query failed: " . $e->getMessage());
    }

    // Fetch calendar file from resources
    $calendar_file = null;
    try {
        $stmt = $pdo->prepare("SELECT * FROM resources WHERE category = ? ORDER BY created_at DESC LIMIT 1");
        $stmt->execute(['Academic']);
        $calendar_file = $stmt->fetch();
    } catch (Exception $e) {
        error_log("Academic Calendar: resources query failed: " . $e->getMessage());
    }

    // Determine current status
    $today = date('Y-m-d');
    $start_date = $settings['academic_calendar_start'] ?? null;
    $end_date = $settings['academic_calendar_end'] ?? null;
    $current_status = '—';
    ...
    if ($start_date && $end_date) {
        if ($today < $start_date) {
            ...
            // Check if today is a holiday/break event
            $is_holiday = false;
    ...
    error_log("Academic Calendar FATAL: " . $e->getMessage());
    $school_name = 'Nex CEC';
    $staff = ['full_name' => 'Staff', 'position' => '', 'department' => ''];
    $events = [];
    $grouped_by_month = [];
    $total_events = 0;
    $upcoming_events = [];
...
<body>
    <?php echo renderStaffSidebar('academic_calendar', $school_name, 0, '', $staff['full_name'] ?? ''); ?>

    <div class="staff-main">
        <div class="top-bar">
            <div>
                <h2><i class="fas fa-calendar-alt"></i> Academic Calendar</h2>
                <p class="subtitle">View all scheduled events, holidays, and term activities</p>
            </div>
        ...
        <div class="cal-summary">
            <div class="stat-card">
                ...
                <div class="info">
                    <h3><?php echo $start_date ? date('M j, Y', strtotime($start_date)) : '-'; ?></h3>
                    <p>Start Date</p>
                </div>
            </div>
            <div class="stat-card">
                ...
                <div class="info">
                    <h3><?php echo $end_date ? date('M j, Y', strtotime($end_date)) : '-'; ?></h3>
                    <p>End Date</p>
                </div>
            </div>
            <div class="stat-card">
        ...
        <div class="upcoming-strip">
            ...
            <?php $days = daysUntil($ue['event_date']); ?>
