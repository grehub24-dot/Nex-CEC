...
<head>
    ...
    <style>
        .payslip { max-width: 800px; margin: 30px auto; background: #fff; border: 2px solid #1a5276; border-radius: 8px; padding: 30px; }
...
<body>
    ...
    <div class="payslip">
        <div class="payslip-header">
            ...
            <h2>PAY SLIP — <?php echo $month_name . ' ' . $payroll['year']; ?></h2>
        </div>

        <div class="payslip-info">
            <div><strong>Staff ID:</strong> <?php echo htmlspecialchars($payroll['staff_id']); ?></div>
            <div><strong>Name:</strong> <?php echo htmlspecialchars($payroll['full_name']); ?></div>
            <div><strong>Position:</strong> <?php echo htmlspecialchars($payroll['position']); ?></div>
            <div><strong>Department:</strong> <?php echo htmlspecialchars($payroll['department'] ?? 'N/A'); ?></div>
            <div><strong>Bank:</strong> <?php echo htmlspecialchars($payroll['bank_name'] ?? 'N/A'); ?></div>
            <div><strong>Account:</strong> <?php echo htmlspecialchars($payroll['account_number'] ?? 'N/A'); ?></div>
        </div>
        ...
        <table class="payslip-table">
            ...
            <tbody>
                <tr>
                    ...
                    <td>SSNIT</td>
                    <td style="text-align: right; color: #e74c3c;"><?php echo number_format($payroll['ssnit_deduction'], 2); ?></td>
                </tr>
                <tr>
                ...
                if ($salary_struct) {
                    if ((float)$salary_struct['transport_allowance'] > 0) {
                        echo "<tr><td>Transport Allowance</td><td style='text-align: right;'>" . number_format($salary_struct['transport_allowance'], 2) . "</td>";
                    }
                    if ((float)$salary_struct['other_allowances'] > 0) {
                        echo "<td>Other Allowances</td><td style='text-align: right;'>" . number_format($salary_struct['other_allowances'], 2) . "</td></tr>";
                    } elseif ((float)$salary_struct['transport_allowance'] <= 0) {
                        echo "<td></td><td></td></tr>";
                    } else {
                        echo "</tr>";
                    }
                ...
                <tr class="total-row">
                    ...
                    <td style="text-align: right;"><strong><?php echo number_format($payroll['gross_pay'], 2); ?></strong></td>
                    <td><strong>TOTAL DEDUCTIONS</strong></td>
                    <td style="text-align: right; color: #e74c3c;"><strong><?php echo number_format($payroll['total_deductions'], 2); ?></strong></td>
                </tr>
            </tbody>
        </table>
        ...
        <div style="background: linear-gradient(90deg, #1a5276, #2e86c1); color: white; text-align: center; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <h2 style="margin: 5px 0 0 0; font-size: 2rem;">GHS <?php echo number_format($payroll['net_pay'], 2); ?></h2>
        </div>
        ...
        <div style="font-size: 0.85rem; color: #666; margin-bottom: 20px;">
            ...
            <?php elseif ($payroll['status'] === 'paid'): ?>
                <span style="color: #004085;">Paid on <?php echo date('M d, Y', strtotime($payroll['pay_date'])); ?></span>
            <?php else: ?>
